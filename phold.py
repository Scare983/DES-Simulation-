...
import numpy as np
argumentHash = {'SIM_TIME': None, 'isFixedLatency': False, 'maxLatency': None, 'numLP':2, 'messagePopulation':None}
...
class LPS:
    ...
    def generateInitialEvent(self):
        ...
        self.state = 0
        #set dest to random location.
        self.setDstLP(np.random.randint(low=0, high=argumentHash['numLP']))
    ...
    def generateXEvents(self, number):
        for x in range(number):
            self.generateInitialEvent()
...
def prossargs():
    try:
        opt, args = getopt.getopt(sys.argv[1:], "s:fp:m:k:")
    ...
    for opts, arg in opt:
        if opts == '-s':
            ...
            if not arg.isdigit():
                ...
                print('cannot use <= 0 balls')
            else:
                argumentHash['messagePopulation'] = int(arg)
...
def usage():
    ...
    print(
        "python phold.py -s 'INT-TIME' [-f optional ] -k 'INT -upper Latency'")
    exit(2)
...
numberToSchedule = int(np.floor(argumentHash['messagePopulation']/argumentHash['numLP']))
...
LPS.setSim(s)
for a in listOfLp:
    a.generateXEvents(numberToSchedule)
s.simulate()
print('number of total trips: {}'.format(s.getNumTotalTrips()))